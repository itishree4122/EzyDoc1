...
const BookingScreen = ({route}) => {
  const { doctor_name, specialist, clinic_name, clinic_address, doctor_user_id, experience, patientId } = route.params;
...
const handleSubmit = async () => {
  try {
    ...
    const appointmentData = {
      ...
      shift: selectedSlot ? selectedSlot.toLowerCase() : '',
...
const handleDateSelection = (item) => {
  ...
  const paddedDate = String(item.date).padStart(2, '0');
  const monthNumber = monthMap[month]; // Convert month name to number
  const formattedDate = `${year}-${monthNumber}-${paddedDate}`;
  ...
  {dates.map((item, index) => (
    <TouchableOpacity
      ...
      onPress={() => handleDateSelection(item)}>
          ...
          <View style={[
            styles.dateBox,
            typeof selectedDate === 'string' && parseInt(selectedDate.split("-")[2], 10) === item.date && styles.selectedBox
          ]}>


            <Text style={[
              styles.dateNumber,
              typeof selectedDate === 'string' && parseInt(selectedDate.split("-")[2]) === item.date && styles.selectedText
            ]}>
              {item.date}
            </Text>
      ...
      <View style={styles.modalContent}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          ...
          <TextInput
            ...
            value={selectedSlot ? selectedSlot.toLowerCase() : ''}
            editable={false}
          />
          ...
          <TextInput
            ...
            value={selectedTime ? selectedTime.split(' ')[0] : ''} // Remove AM/PM part
            editable={false}
          />
